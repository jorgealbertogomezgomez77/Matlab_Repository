function [MFCC2] = mfcc_rasta_delta_pkm_v1(d,Fs,Nomfccs,Nbands,Framesize,FrameShift,Del,DelDel,NDL)

% Del==1     To Compute Delta Features
% DelDel=1   To Compute Delta Delta Features
% NDL        No of frames considered for computing dynamic features

% d=d-mean(d);

MFCC= mfcc_rasta_V1(d,Fs,'wintime',Framesize/1000,'hoptime',FrameShift/1000,'numcep',Nomfccs,'nbands', Nbands);

mfcc_mcs_lift=MFCC';

%-----------------------------DYNAMIC FEATURES - ADDED BY PKM ON 03/06/08------------------------------

if(Del==1)

    [DMFCC DDMFCC]=computedeltas_V2(mfcc_mcs_lift,Nomfccs,NDL);     % Usage Refer GMM_README_PKM.pdf   
    for i=1:size(mfcc_mcs_lift,1)
    
        if(Del==1 && DelDel==0)
            MFCC2(i,:)=[mfcc_mcs_lift(i,:) DMFCC(i,:)];
        elseif(Del==1 && DelDel==1)
            MFCC2(i,:)=[mfcc_mcs_lift(i,:) DMFCC(i,:) DDMFCC(i,:)];
        end
...
[wintime, hoptime, numcep, lifterexp, sumpower, preemph, dither, ...
 minfreq, maxfreq, nbands, bwidth, dcttype, fbtype, usecmp, modelorder] = ...
    process_options(varargin, 'wintime', 0.020, 'hoptime', 0.010, ...
